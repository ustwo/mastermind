"""
    ...
    expected input, a list of values or a list of tuples.  The first case
    applies values in strict order, the second by key.
"""
...
def varlist(template):
    """
        Returns the ordered list of all variables in a template.  It discards
        any operator knowledge from each expression.
    """
...
def expand_segments(tpl, segments, partial=False):
    """
        When partial is False and there are no seguments left it follows the
        RFC so the result is empty.  But, when partial is True, the expression
        is kept intact so you can apply multiple times the function with
        different segments:
...
def expand_pairs(tpl, pairs, partial=False):
    """
        When partial is False and there are no matching pairs it follows the
        RFC so the result is empty.  But, when partial is True, the expression
        is kept intact so you can apply multiple times the function with
        different pairs:
    ...
    def take_leftovers(varlist, pairs):
        return [var for var in varlist
                if not any([var == x for (x, _) in pairs])]
